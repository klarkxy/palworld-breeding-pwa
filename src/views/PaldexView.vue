...
import { elementName, formatDex, usePalData, workName } from "@/composables/usePalData";
...
<template>
  <main class="page-shell">
    <PageIntro eyebrow="图鉴架 / Paldex shelf" title="查数据，也直接去配种" description="按名称、编号、属性和工作适应性找到帕鲁；每一页都能直接带入计算器。" />
    <DataState :is-loading :error @retry="load">
      <section class="filter-bar filter-bar--paldex" aria-label="筛选图鉴">
        <label class="field filter-search"><span class="field__label">搜索图鉴</span><input v-model="query" type="search" placeholder="中文、English、编号或 ID" /></label>
