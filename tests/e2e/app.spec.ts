...
test("图鉴已拥有标记支持旧地址、排序、刷新和库存路线", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/#\/paldex\?state=1&sort=attack&view=owned$/);
});
...
test("表单跨路由、刷新和新页面恢复，路线结果按条件重新计算", async ({ page, context }) => {
  ...
  await expect(palDrawer(restored, "吓丝妮").getByRole("radio", { name: "0 星", exact: true })).toBeChecked();
...
test("道具图鉴与材料计算器展开蛋糕的完整真实配方", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/#\/items\/calculator\?state=1&target=Cake&qty=2$/);
